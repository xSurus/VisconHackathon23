...
import type { Category } from "../../util/schemas";
	...
	switch (req.method) {
		case "GET":
			try {
				...
				let rows: Category[] = result.rows.map((x) => x.name);
		...
		case "PUT":
			...
			try {
				let result = await db.query(
					"INSERT INTO Category(name) VALUES ($1::text) ON CONFLICT DO NOTHING",
					[query.name]
				);
			} catch (e) {}
		...
		case "DELETE":
			...
			try {
				let result = await db.query(
					"DELETE FROM Category WHERE name = $1::text",
					[query.name]
				);
			} catch (e) {}
			return res.status(200).send(undefined);
